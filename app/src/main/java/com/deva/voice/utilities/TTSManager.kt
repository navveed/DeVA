...
class TTSManager private constructor(private val context: Context) : TextToSpeech.OnInitListener {
    ...
    private suspend fun speak(text: String) {
        try {
            // Use Android's built-in TTS (no API key required)
            isNativeTtsInitialized.await()
            
            // Create a deferred to wait for speech completion
            val speechComplete = CompletableDeferred<Unit>()
            ...
            Log.d("TTSManager", "Native TTS completed for: ${text.take(50)}...")
            
        } catch (e: Exception) {
            if (e is CancellationException) throw e
            Log.e("TTSManager", "Native TTS failed: ${e.message}", e)
        }
    }
