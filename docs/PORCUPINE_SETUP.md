...
1. **STT Engine (Default)**: Uses Android's Speech-to-Text to listen for "DeVA"
2. **Porcupine Engine**: Uses Picovoice's Porcupine library to listen for "DeVA" (custom wake word)
...
2. Select your preferred wake word engine:
   - **STT Engine**: Listens for "DeVA" (works offline)
   - **Porcupine Engine**: Listens for "DeVA" (more accurate, requires internet)
3. Tap "ENABLE WAKE WORD" to start listening
4. Say the wake word to activate the app

### 3. Custom Wake Words

The app is already configured to use a custom "DeVA" wake word. The `DeVA_en_android_v3_0_0.ppn` file is included in the assets folder.
