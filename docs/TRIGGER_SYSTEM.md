...
## Overview
The Trigger System allows DeVA to be initiated from various entry points, enabling proactive and automated task execution. Instead of waiting for a direct voice command, users can configure triggers, such as a specific time of day or an incoming notification from another app, to launch a DeVA task. This infrastructure is designed to be flexible and extensible for future trigger types.
...
- If it's a notification trigger, no alarm is scheduled. The `DeVANotificationListenerService` is responsible for monitoring it.
...
1.  The `DeVANotificationListenerService` is constantly running in the background (once enabled by the user).
...
- **Responsibilities**:
  - Receives `Intent` broadcasts from both `AlarmManager` and `DeVANotificationListenerService`.
...
#### `DeVANotificationListenerService` (NotificationListenerService)
...
│   └── TriggersActivity.kt
├── BootReceiver.kt
├── DeVANotificationListenerService.kt
├── PermissionUtils.kt
├── Trigger.kt
├── TriggerManager.kt
└── TriggerReceiver.kt
...
- `android.permission.BIND_NOTIFICATION_LISTENER_SERVICE`: Required for the `DeVANotificationListenerService` to read notifications. This permission must be granted manually by the user in system settings.
