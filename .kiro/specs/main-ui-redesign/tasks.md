...
- [ ] 1. Create state management system for delta symbol
  - Create `DeVAState` enum with IDLE, LISTENING, PROCESSING, SPEAKING, ERROR states
  - Create `DeVAStateManager` class to monitor ConversationalAgentService status
...
- [x] 3. Create Pro upgrade banner component
  - Design banner layout with "Get more with DeVA Pro" text and "Upgrade" button
...
- [x] 4. Modify main activity layout structure
  - Remove "Hey DeVA here" and "Assistant at your command" text elements
...
- [x] 5. Preserve essential UI elements
  - Keep "How to wake up DeVA" link at bottom with existing styling
...
- [ ] 10. Test state transitions and visual feedback
  - Write unit tests for DeVAStateManager state logic
